...
knows (Looks right, Needs your decision, Problem found) and one plain sentence
saying what the figure is, and then shows the evidence before it asks anything:
the captures tell the story situation by situation - a grid of tiles, each headed
by the situation's own title, the filters that produced it, its key figures and
the screenshot itself (click to open it full size) - then the table of what the
screen showed, then the checks an analyst would make by hand (does the reset land
back on the baseline, did the change move anything, did it move the right way,
does the card equal the table or the ratio of its two inputs, did our own
calculation agree), with the checks that passed collapsed behind one line so a
failure is never buried in green ticks. Only then come the questions, each in its
own bordered block with a one-line reason, a thumbnail strip of the situations it
concerns and, where a claim can answer it, the two buttons that do. A page-level
card compares figures that carry the same name on more than one component; a
regression case leads with one sentence a manager can act on and badges every
component with what happened to it.
...
.ask .thumbs{display:flex;gap:10px;flex-wrap:wrap;margin:0 0 12px}
.ask .thumb{margin:0;flex:1 1 220px;min-width:180px;max-width:340px}
.thumb a.shot-link{display:block;border:1px solid #d9dfe2;border-radius:6px;overflow:hidden;background:#fff}
.thumb img{display:block;width:100%;height:auto}
.thumb figcaption{font-size:12.5px;color:#586066;margin:4px 0 0;line-height:1.35}
.thumb figcaption b{color:#1f2528;font-weight:600}
...
@media print{
 ...
 .card,.ask,.sit,.thumb,.toc{break-inside:avoid;page-break-inside:avoid}
 .thumbs{display:flex!important}.thumb img{max-width:100%}
...
GENERATOR = 'analytics-qa review_form'
...
def situation_index(observations, component_id, report_dir_rel):
    ...
    for position, observation in enumerate(observations):
        ...
        filters = analyst_view.filter_phrase(observation)
        index[observation['label']] = {
            'number': number, 'situation': situation, 'filters': filters,
            'anchor': analyst_view.situation_anchor(component_id, position), 'src': source,
            'caption': f'{number} · {situation} — {filters}'}
    return index
...
def thumb_figure(src, caption, title, detail=''):
    """One small clickable picture with a caption that says which situation it is."""
    esc = html.escape
    link = (f'<a class="shot-link" href="{esc(src)}" target="_blank" rel="noopener" '
            f'data-caption="{esc(caption)}" title="{esc("Open " + title)}" aria-label="{esc("Open " + title)}">'
            f'<img src="{esc(src)}" alt="{esc(title)}" loading="lazy"></a>')
    tail = f'<br>{esc(detail)}' if detail else ''
    return f'<figure class="thumb">{link}<figcaption><b>{esc(title)}</b>{tail}</figcaption></figure>'


def question_thumbs(labels, index):
    """The situations one question is about, shown above it so the evidence comes first."""
    tiles = []
    for label in dict.fromkeys(labels or []):
        entry = (index or {}).get(label)
        if not entry or not entry.get('src'):
            continue
        tiles.append(thumb_figure(entry['src'], entry['caption'],
                                  f'{entry["number"]} · {entry["situation"]}', entry.get('filters', '')))
    return f'<div class="thumbs">{"".join(tiles)}</div>' if tiles else ''


def cross_thumbs(links, report_dir_rel):
    """The same strip for a page-level question, which carries its own screenshot targets."""
    tiles = []
    for link in links or []:
        if not link.get('screenshot'):
            continue
        tiles.append(thumb_figure(report_dir_rel + '/' + link['screenshot'], link['label'], link['label']))
    return f'<div class="thumbs">{"".join(tiles)}</div>' if tiles else ''
...
def question_block(ask, index=None):
    """One question in its own bordered block: the evidence, the question, then the answer.

    A reader meets the picture before the question about it: the situations this
    question concerns are shown as a small strip above the words, and the "view"
    links stay so a check and a question still open the same lightbox.
    """
    esc = html.escape
    index = index or {}
    body = ask.get('thumbs_html') or question_thumbs(ask.get('capture_labels'), index)
...
def across_card(views, report_dir_rel, catalog=None):
    ...
    for ask, check in zip(asks, [c for c in checks if c['status'] == 'fail']):
        ...
        ask['thumbs_html'] = cross_thumbs(check.get('links'), report_dir_rel)
    body = ''
    if checks:
...
def prepare(case_path, out_path, embed_images=False):
    ...
    if out.is_relative_to(case_dir):
        raise ValueError('Write the sign-off page outside the sealed case, beside it')
    if out.exists() and GENERATOR not in out.read_text(encoding='utf-8', errors='replace'):
        # Regenerating our own page is routine - a newer renderer, a re-read case. Anything
        # else at that path is someone's file, and the page is not worth overwriting it.
        raise ValueError(f'{out} exists and was not produced by this tool; choose another output')
